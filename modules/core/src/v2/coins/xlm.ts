...
import { coinTokenPatternSeparator } from '../coinFactory';
...
export class Xlm extends BaseCoin {
  ...
  public readonly tokenPatternSeparator = '-'; // separator for token code and issuer
   ...
   * @param {Asset} asset - instance of Stellar Asset
   */
  getTokenFromStellarAsset(asset) {
    if (!(asset instanceof stellar.Asset)) {
      throw new Error('expected asset to be an instance of Stellar Asset');
    }
    ...
    return `${this.getChain()}${coinTokenPatternSeparator}${code}${this.tokenPatternSeparator}${issuer}`;
  }
    ...
    return co(function *() {
      ...
      _.forEach(tx.operations, op => {
        if (op.type === 'createAccount' || op.type === 'payment') {
          ...
          const coin = this.getTokenFromStellarAsset(asset); // coin or token id
          ...
          operations.push({
            type: op.type,
            coin: this.getTokenFromStellarAsset(op.line),
            asset: op.line,
            limit: op.limit,
          });
        }
